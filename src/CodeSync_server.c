...
unsigned WINAPI HandleClient(void* arg) {
    ...
    while ((strLen = recv(hClntSock, msg, MAX_MSG_LEN, 0)) != 0 && strLen != -1) {
        ...
        } else if (strcmp(command, "HISTORY_CODE_REQUEST") == 0) {
            char *commitIdStr = strtok(NULL, "\t");  // 클라이언트로부터 전달받은 CommitID 추출
            if (commitIdStr != NULL) {
                // 검색할 스니펫 파일명을 결정하기 위한 준비
                char userId[BUF_SIZE] = "";
                char snippetFile[BUF_SIZE] = "";

                // 해당 CommitID에 대응되는 사용자 ID를 찾기 위해 history.txt 파일을 탐색
                FILE *fp_hist = fopen("history.txt", "r");
                if (fp_hist) {
                    char line[BUF_SIZE*4];
                    while (fgets(line, sizeof(line), fp_hist)) {
                        // history.txt에서 각 줄은 "CommitID\tUserID\t제목\t메시지\t시간" 형식이라고 가정
                        char *foundId = strtok(line, "\t");
                        if (foundId && strcmp(foundId, commitIdStr) == 0) {
                            // CommitID가 일치하면 다음 토큰인 UserID를 가져옴
                            char *foundUser = strtok(NULL, "\t");
                            if (foundUser) {
                                strcpy(userId, foundUser);
                            }
                            break;
                        }
                    }
                ...
                // 사용자 ID를 정상적으로 찾았다면 해당 사용자의 스니펫 파일명을 설정
                if (strlen(userId) > 0) {
                    sprintf(snippetFile, "snippets_%s.txt", userId);
                } else {
                    // 사용자 ID를 찾지 못한 경우 에러 메시지 전송
                    const char *err = "커밋을 찾을 수 없거나, 유효하지 않는 commitID 입니다.\n##END##\n";
                    send(hClntSock, err, strlen(err), 0);
                    continue;
                }

                // 스니펫 파일을 열고 해당 CommitID의 코드 블록을 검색
                FILE *fp_snip = fopen(snippetFile, "r");
                if (fp_snip) {
                    char sendBuf[BUF_SIZE*10] = "";  // 클라이언트에 보낼 코드 누적 버퍼
                    char line[BUF_SIZE];
                    int inBlock = 0;
                    while (fgets(line, sizeof(line), fp_snip)) {
                        // 스니펫 파일에서 CommitID가 포함된 줄을 커밋 블록의 시작으로 간주
                        if (!inBlock) {
                            if (strstr(line, commitIdStr) != NULL) {
                                inBlock = 1;
                                // 헤더 라인은 건너뛰고 다음 줄부터 코드 누적 시작
                                continue;
                            }
                        }
                        if (inBlock) {
                            // 다음 커밋 헤더나 ## 구분자가 나오면 블록 끝으로 간주하고 종료
                            if (strstr(line, "Commit ") == line || strstr(line, "##") == line) {
                                break;
                            }
                            strcat(sendBuf, line);  // 코드 누적
                        }
                    }
                    fclose(fp_snip);

                    // 코드 블록이 비어있으면 에러 메시지 전송
                    if (strlen(sendBuf) == 0) {
                        const char *msg = "해당 커밋 코드를 찾을 수 없습니다.\n##END##\n";
                        send(hClntSock, msg, strlen(msg), 0);
                    } else {
                        // 코드 블록 전송 시작 (선택적으로 응답 헤더 추가 가능)
                        send(hClntSock, "HISTORY_CODE_RESPONSE\n", strlen("HISTORY_CODE_RESPONSE\n"), 0);
                        send(hClntSock, sendBuf, strlen(sendBuf), 0);
                        send(hClntSock, "##END##\n", strlen("##END##\n"), 0);
                    }
                } else {
                    // 스니펫 파일 열기에 실패한 경우 에러 메시지 전송
                    const char *msg = "스니펫 파일 열기에 실패했습니다.\n##END##\n";
                    send(hClntSock, msg, strlen(msg), 0);
                }
            }
